		...
		>
			...
			<div class="flex w-full flex-col bg-white p-4 lg:w-96 dark:bg-gray-900">
				...
				<div class="mb-4 flex items-center gap-3">
					...
					<div class="flex-1">
						<p class="font-medium text-gray-900 dark:text-white">{video.author?.name || 'Unknown'}</p>
				...
				<div class="flex items-center gap-4 border-t border-gray-100 py-3 dark:border-gray-800">
					...
					>
						<Heart class="h-5 w-5" />
						<span class="text-sm">{formatNumber(video.likesCount)}</span>
					</button>
					<button
						class="flex items-center gap-1.5 text-gray-600 transition-colors hover:text-rose-500 dark:text-gray-400"
					>
						<MessageCircle class="h-5 w-5" />
						<span class="text-sm">{formatNumber(video.commentsCount)}</span>
					</button>
					<button
						class="flex items-center gap-1.5 text-gray-600 transition-colors hover:text-rose-500 dark:text-gray-400"
					>
						<Share2 class="h-5 w-5" />
						<span class="text-sm">{formatNumber(video.sharesCount)}</span>
					</button>
					<button
						class="flex items-center gap-1.5 text-gray-600 transition-colors hover:text-rose-500 dark:text-gray-400"
					>
